...
- Create a bucket of `clww-vpc-endpoint` with following properties, 
...
- list the bucket in S3 and content of S3 bucket named "aws s3 ls "clww-vpc-endpoint" via following command

```
aws s3 ls
aws s3 ls clww-vpc-endpoint
```
...
"clww-vpc-endpoint" via following command
```bash
aws s3 ls
aws s3 ls clww-vpc-endpoint-comp
```

- copy the 'Guile.png' and 'Honda.png' files from S3 bucket into the private EC2
```bash
aws s3 cp s3://clww-vpc-endpoint-comp/Guile.png .
aws s3 cp s3://clww-vpc-endpoint-comp/Honda.png .
```
